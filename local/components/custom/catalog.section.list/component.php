...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	if(!empty($arParams["SECTION_ID"]))
	{
		...
		while($rowSections = $rsSections->GetNext()) {
			$arResult["SECTION"][] = $rowSections;
		}
	...
	if(!empty($arResult["SECTION"])) {
		foreach ($arResult["SECTION"] as $k_section => $v_section) {
			if (is_array($v_section)) {
				unset($arFilter["ID"]);
				unset($arFilter["=CODE"]);
				$arFilter["LEFT_MARGIN"] = $v_section["LEFT_MARGIN"] + 1;
				$arFilter["RIGHT_MARGIN"] = $v_section["RIGHT_MARGIN"];
				$arFilter["<=" . "DEPTH_LEVEL"] = $v_section["DEPTH_LEVEL"] + $arParams["TOP_DEPTH"];
				
				$ipropValues = new \Bitrix\Iblock\InheritedProperty\SectionValues($v_section["IBLOCK_ID"], $arResult["SECTION"]["ID"]);
				$arResult["SECTION"][$k_section]["IPROPERTY_VALUES"] = $ipropValues->getValues();
				
				$arResult["SECTION"][$k_section]["PATH"] = [];
				...
				while ($arPath = $rsPath->GetNext()) {
					...
					$arResult["SECTION"][$k_section]["PATH"][] = $arPath;
				}
			}
			else {
				$arResult["SECTION"][$k_section] = ["ID" => 0, "DEPTH_LEVEL" => 0];
				$arFilter["<=" . "DEPTH_LEVEL"] = $arParams["TOP_DEPTH"];
			}
			$intSectionDepth = $v_section['DEPTH_LEVEL'];
			...
			//EXECUTE
			$rsSections = CIBlockSection::GetList($arSort, array_merge($arrFilter, $arFilter), $arParams["COUNT_ELEMENTS"], $arSelect);
			$rsSections->SetUrlTemplates("", $arParams["SECTION_URL"]);
			while ($arSection = $rsSections->GetNext()) {
				...
				$elCount = 0;
				$rsElements = CIBlockElement::GetList([], ['SECTION_ID' => $arSection["ID"], 'PROPERTY_MAKE' => $arParams["MAKE_ID"], "ACTIVE" => "Y"], false, false, ['ID']);
				while ($arElements = $rsElements->GetNext()) {
					/*if ($USER->GetID() == 1) {
						echo $arSection["ID"] . ' - ' . $arElements['ID'] . ' - '. $arParams["MAKE_ID"] .'<br>';
					}*/
					$elCount++;
				}
				//print_r($elCount);echo '<br>';
