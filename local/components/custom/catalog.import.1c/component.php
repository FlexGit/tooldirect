...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	...
	if (!$_SESSION['is_complete']) {
		$wsId = 'c978a97a-27e2-11e6-8108-ac220b889bb9';
		$file = $_SERVER['DOCUMENT_ROOT'] . '/upload/1c_catalog/offers0_1.xml';
		$file2 = $_SERVER['DOCUMENT_ROOT'] . '/upload/1c_catalog/webdata/offers0_1.xml';
		...
		if (file_exists($file) || file_exists($file2)) {
			...
			//echo '<pre>';print_r($xml->ПакетПредложений->Предложения);echo '</pre>';
			
			$arr = (array)$xml->ПакетПредложений->Предложения;
			...
			foreach ($xml->ПакетПредложений->Предложения->Предложение as $offer) {
				...
				if (!empty($offer->Артикул))
					$article = trim($offer->Артикул);
				
				if (!empty($offer->Цены->Цена->ЦенаЗаЕдиницу))
					$price = (array)$offer->Цены->Цена->ЦенаЗаЕдиницу;
				
				//echo '<pre>';print_r((array)$offer->Склад);echo '</pre>';
				
				foreach ($offer->Склад as $ws) {
					$ws = (array)$ws;
					//echo '<pre>';print_r($ws);echo '</pre>';
					if ((string)$ws['@attributes']['ИдСклада'] == $wsId) {
						//echo $article.' - '.$ws['@attributes']['КоличествоНаСкладе'].'<br>';
						$quantity = $ws['@attributes']['КоличествоНаСкладе'];
					}
				}
				
				//if (empty($article) || empty($quantity) || empty($price)) continue;
				...
				//echo $offer->Ид.' - '.$offer->Артикул.' - '.$offer->Цены->Цена->ЦенаЗаЕдиницу.' - '.$offer->Количество.'<br>';
			}
			...
			if (!empty($arr)) {
				foreach ($arr as $v) {
					//echo '!'.$v['article'].'!<br>';
					//if ($v['article'] != '124.123583S') continue;
					
					//echo $v['article'].'<br>';
					
					// поиск товара на сайте по артикулу
					$arFilter = ["IBLOCK_ID" => CATALOG_BLOCK_ID, "PROPERTY_ARTICLE" => $v['article']];
					...
					if ($ob = $res->GetNextElement()) {
						...
						if (trim($v['price']) == '') $v['price'] = 0;
						if (trim($v['quantity']) == '') $v['quantity'] = 0;
						...
						$res = CPrice::GetList([], ["PRODUCT_ID" => $elId, "CATALOG_GROUP_ID" => 1]);
						if ($arr = $res->Fetch()) {
							//echo $elId.' - '.$v['quantity'].'<br>';
							CCatalogProduct::Update($elId, ['QUANTITY' => $v['quantity']]);
				...
				if (defined('BX_COMP_MANAGED_CACHE') && is_object($GLOBALS['CACHE_MANAGER'])) {
					$GLOBALS['CACHE_MANAGER']->ClearByTag('iblock_id_' . CATALOG_BLOCK_ID);
				}
				/*$files = glob($_SERVER['DOCUMENT_ROOT'] . '/upload/1c_catalog/*');
				foreach ($files as $file) {
					if (is_file($file)) {
						unlink($file);
					}
				}*/
				$_SESSION['is_complete'] = 1;
			}
		}
	}
